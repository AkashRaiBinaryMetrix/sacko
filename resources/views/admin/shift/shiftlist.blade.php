...
<div class="container-fluid flex-grow-1 container-p-y">
	<h4 class="font-weight-bold py-3 mb-0">Shift List ({{ count($datacountlists)}})</h4>
	<div class="text-muted small mt-0 mb-4 d-block breadcrumb">
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="feather icon-home"></i></a></li>
			<li class="breadcrumb-item"><a href="{{ route('admin.attendance.index') }}">Shift List</a></li>
		</ol>
	</div>
	<div class="row">
							...
							<table id="report-table" class="table table-bordered table-striped mb-0">
								...
								<tbody>
									...
									<tr>
										...
										<td>{{$val->shift_start_date}}</td>
										<td>{{$val->shift_end_date}}</td>
										<td>{{$val->shift_start_time}}</td>
										<td>{{$val->shift_end_time}}</td>
										<td>
											@if($val->status=='1')<span class="badge badge-success">Active</span>@else<span class="badge badge-danger">InActive</span>@endif
										</td>
										<td>
											<a data-size="lg"  class="btn btn-icon btn-outline-primary" data-toggle="tooltip" title="View" href="{{ url('admin/projectlist/edit',$val->id) }}">
												<i class="feather icon-eye"></i>
											</a>
											<a data-size="lg"  class="btn btn-icon btn-outline-primary" data-toggle="tooltip" title="View" href="{{ url('admin/projectlist/delete',$val->id) }}">
