...
<div class="container-fluid flex-grow-1 container-p-y">
	<h4 class="font-weight-bold py-3 mb-0">	@lang('message.feedback_list') ({{ count($feedback)}})</h4>
	<div class="text-muted small mt-0 mb-4 d-block breadcrumb">
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="#"><i class="feather icon-home"></i></a></li>
			<li class="breadcrumb-item"><a href="{{ route('admin.feedback.index') }}">@lang('message.feedback')</a></li>
		</ol>
	</div>
	<div class="row">
							...
							<table id="report-table" class="table table-bordered table-striped mb-0">
								<thead>
									<tr>
										...
										<th>@lang('message.feedback_type')</th>
								...
								@foreach($feedback as $val)
									<tr>
										...
										<td>{{ $val->username }}</td>
										<td>{{ $val->feedback_type }}</td>
										<td>
											<a data-size="lg"  class="btn btn-icon btn-outline-success" data-toggle="tooltip" title="Edit" href="{{ url('admin/feedback/view',$val->id) }}">
	...
	for(i=0;i<element_lenght;i++)
	{
		if(formid.elements[i].name=="mul_del[]")
		{
			if(formid.elements[i].checked==true)
			{
				...
				{
					$("#formid").attr("action", "{{url('admin/feedback/delete')}}");
					this.formid.submit();
					break;
				}
			}	
